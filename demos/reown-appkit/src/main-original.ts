...
class ArenaHeadlessWallet implements HeadlessWallet {
  ...
  // Removed isDisconnected flag - using accounts.length for state consistency
  ...
  // Centralised disconnect logic - matches MetaMask behavior exactly with timing delays
  performDisconnect(source: string) {
    // Clear accounts first - using accounts.length as source of truth
    this.accounts = []

    addLog(`✅ Disconnecting EVM wallet (${source}) - clearing accounts`)

    // MetaMask pattern: emit accountsChanged with empty array first
    // This is the primary signal that the wallet has disconnected
    // Add timing delay to prevent race conditions
    setTimeout(() => {
      addLog(`📡 Emitting accountsChanged with empty array`)
      this.emit('accountsChanged', [])
    }, 50)

    // Then emit disconnect event with proper error code after a delay
    // MetaMask uses error code 1013 for user-initiated disconnects
    setTimeout(() => {
      addLog(`📡 Emitting disconnect event`)
    ...
    // DO NOT call appKit.disconnect() - let AppKit react to our events
    // This matches MetaMask's behavior where the wallet just emits events
    // and the dapp/framework handles the state change

    // Force UI update with delay to allow events to propagate
    setTimeout(() => {
      addLog(`🔄 Forcing UI update after disconnect`)
      updateUI()
    }, 150)
  }
    ...
    switch (method) {
      case 'eth_requestAccounts': {
        if (this.accounts.length === 0) {
          ...
          setTimeout(() => {
            addLog(`📡 Emitting connect event`)
            this.emit('connect', { chainId: this.currentChain })
          }, 50)

          // Then emit accountsChanged
          setTimeout(() => {
            addLog(`📡 Emitting accountsChanged with ${this.accounts.length} accounts`)
            this.emit('accountsChanged', this.accounts)
          }, 100)

          // Add connection liveliness signal - emit chainChanged after connection
          setTimeout(() => {
            addLog(`📡 Emitting chainChanged liveliness signal: ${this.currentChain}`)
            this.emit('chainChanged', this.currentChain)
          }, 150)

          // Update UI
          setTimeout(() => {
            addLog(`🔄 Updating UI after connection`)
            updateUI()
          }, 200)
        }
      ...
      case 'eth_accounts': {
        addLog(`📋 eth_accounts returning ${this.accounts.length} accounts`)
        return this.accounts
      }
      ...
      case 'eth_sendTransaction': {
        if (this.accounts.length === 0) {
          addLog(`❌ eth_sendTransaction: No accounts connected`)
      ...
      case 'wallet_requestPermissions': {
        ...
        if (eth_accounts) {
          ...
          if (this.accounts.length === 0) {
            this.accounts = getEVMAddresses()
            addLog(`✅ Permission granted for eth_accounts - ${this.accounts.length} accounts`)

            // Emit connect event first
            setTimeout(() => {
              addLog(`📡 Emitting connect event from permissions`)
              this.emit('connect', { chainId: this.currentChain })
            }, 50)

            // Then emit accountsChanged
            setTimeout(() => {
              addLog(`📡 Emitting accountsChanged from permissions`)
              this.emit('accountsChanged', this.accounts)
            }, 100)

            // Add connection liveliness signal
            setTimeout(() => {
              addLog(`📡 Emitting chainChanged liveliness signal from permissions: ${this.currentChain}`)
              this.emit('chainChanged', this.currentChain)
            }, 150)

            // Update UI
            setTimeout(() => {
              addLog(`🔄 Updating UI after permission grant`)
              updateUI()
            }, 200)
          }
        }
      ...
      case 'wallet_getPermissions': {
        if (this.accounts.length === 0) {
          addLog(`🔐 wallet_getPermissions: no accounts, returning empty permissions`)
          return []
        }
        addLog(`🔐 wallet_getPermissions: returning eth_accounts permission`)
        return [{ parentCapability: 'eth_accounts' }]
      }
...
class ArenaMockSolanaWallet implements Wallet {
  ...
  async #connect() {
    if (this.connectedAccounts.length === 0) {
      ...
      // Emit change event synchronously (matching MetaMask's pattern)
      addLog(`📡 Solana emitting change event with new account`)
  ...
  async #disconnect() {
    // Prevent redundant disconnect if already disconnected
    if (this.connectedAccounts.length === 0) {
      addLog('🟣 Solana already disconnected')
      return
    }

    // Clear accounts immediately
    this.connectedAccounts = []
    addLog('🟣 Solana wallet disconnected - clearing accounts')

    // Emit change event synchronously (matching MetaMask's pattern)
    addLog(`📡 Solana emitting change event with empty accounts`)
  ...
  get publicKey() {
    return this.connectedAccounts.length > 0 ? this.keypair.publicKey : null
  }
...
// performDisconnect is now public and can be called externally
...
// Expose AppKit and adapters globally for debugging and disconnect handling
;(window as any).appKit = appKit
;(window as any).ethersAdapter = ethersAdapter
;(window as any).solanaAdapter = solanaAdapter
...
try {
  ...
  const originalSubscribe = (appKit as any).subscribeState || (appKit as any).subscribe;
  if (originalSubscribe) {
    ...
    const stateHandler = (state: any) => {
      ...
      const walletConnected = headlessWallet['accounts'].length > 0
      addLog(`🔍 AppKit state monitor - AppKit disconnected: ${appKitDisconnected}, wallet connected: ${walletConnected}`)
...
setInterval(() => {
  ...
  if (connectButton) {
    ...
    const walletConnected = headlessWallet['accounts'].length > 0
    addLog(`🔍 Button state check - wallet connected: ${walletConnected}, button text: "${buttonText}"`)
...
function updateUI() {
  ...
  // Check wallet connection status - use accounts.length as source of truth
  const walletEVMConnected = headlessWallet && headlessWallet['accounts'].length > 0
  addLog(`🔍 UI Update - EVM connected: ${walletEVMConnected}, accounts: ${headlessWallet ? headlessWallet['accounts'].length : 0}`)
  ...
  const buttons = ['sign-message', 'sign-typed-data', 'send-transaction', 'switch-to-polygon', 'switch-to-ethereum', 'get-capabilities', 'evm-disconnect']
  buttons.forEach(id => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('solana-disconnect')?.addEventListener('click', async () => {
    try {
      ...
      // Remove duplicate log - the #disconnect method already logs this
      document.getElementById('solana-result')!.innerHTML = `
        <div class="info-box">
          <h4>Solana Disconnected</h4>
        </div>
  ...
  // EVM Disconnect button
  const evmDisconnectBtn = document.getElementById('evm-disconnect') as HTMLButtonElement
  evmDisconnectBtn?.addEventListener('click', () => {
    addLog('🔓 EVM disconnect requested')
    headlessWallet.performDisconnect('User disconnect')
  })
